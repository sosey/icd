...
import icd.web.shared.IcdModels.{SubscribeModelInfo, TelemetryModel}
...
object ComponentDataReporter {
  ...
  case class SubscribeInfo(subSomponent: String, pubComponent: String, pubSubsystem: String, name: String, rate: Double)
  ...
  def listPublishManifest(db: IcdDb, subsystem: String): Unit = {
    val subscribeInfo = db.query.getPublishInfo(subsystem)
    val subsystemItems = mutable.MutableList[SubscribeInfo]()
    subscribeInfo.foreach { componentSubscribeInfo =>
      println(s" ----  ${componentSubscribeInfo.componentName} ----- ")
      val componentModel = db.query.getComponentModel(subsystem, componentSubscribeInfo.componentName)
      val componentItems = mutable.MutableList[SubscribeInfo]()
      componentModel.foreach { cm =>
        db.query.getSubscribeModel(cm).foreach { csm =>
          if (csm.eventList.nonEmpty) {
            println("--- Event Data")
            componentItems ++= processSubscribeList(csm.eventList, componentSubscribeInfo.componentName)
          }
          if (csm.telemetryList.nonEmpty) {
            println("--- Telemetry Data")
            componentItems ++= processSubscribeList(csm.telemetryList, componentSubscribeInfo.componentName)
          }
          if (csm.eventStreamList.nonEmpty) {
            println("--- EventSteam Data")
            componentItems ++= processSubscribeList(csm.eventStreamList, componentSubscribeInfo.componentName)
          }
        }
      }
      println(s"COMPONENT SUMMARY: ${componentSubscribeInfo.componentName}")
      printSubscriptionSummary(componentItems)
      subsystemItems ++= componentItems
    }
    println("SUBSYSTEM SUMMARY")
    println("-----------------")
    printSubscriptionSummary(subsystemItems)  }
  ...
  private def printEventSummary(items: mutable.MutableList[EventInfo]): Unit = {
    val eventMap = new mutable.HashMap[Double, List[EventInfo]]() {
      override def default(key: Double): List[EventInfo] = List[EventInfo]()
    }
    items.foreach { item =>
      val newList = eventMap(item.rate) :+ item
      eventMap += ((item.rate, newList))
    }
    val sortedMap = eventMap.toSeq.sortBy(_._1)
    sortedMap.foreach(item => {
      println(s"Rate ${item._1} Hz: ${item._2.length} item(s)")
      printEventInfos(item._2)
    })
  }
  ...
  private def processSubscribeList(items: List[SubscribeModelInfo], component: String): List[SubscribeInfo] = items.map(displaySubscription(_, component))
  ...
  private def displaySubscription(item: SubscribeModelInfo, component: String): SubscribeInfo = {
    SubscribeInfo(component, item.component, item.subsystem, item.name, item.requiredRate)
  }
